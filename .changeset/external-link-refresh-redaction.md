...
`https://host/<redacted>?…` 로 바꾸고 `rel`·`type`·`hreflang`·`media`·`title`·`title*` 파라미터는
원문으로 남깁니다 — pagination 진단(`rel="next"`)은 그대로 보입니다. 그 밖의 파라미터(`anchor`
등)는 값을 `[redacted]` 로 씁니다. `refresh` 는 지연 초를 남기고 `url=` 의 URL 만 지웁니다.
