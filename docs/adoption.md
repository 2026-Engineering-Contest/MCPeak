...
- 최종 갱신: 2026-09-10 (§1.8 추가). 그 전 갱신은 2026-09-08 (§1.6 · §1.7 · §5). 그 밖의 절은 2026-08-18 기준
...
| 적용한 자체 서버 | — | 2개 (`examples/weather-server`, `examples/zod-notes-server`) |
...
2행에서 **2026-08-23 이전에** 확인한 것은 **배선이 없다는 것까지**였다. 배선이 없어 시도
자체가 불가능했다. 배선이 들어온 지금도 **공개된 원격 서버에 실제로 연결해 본 적은 없고, 그
동작은 여전히 미측정이다** — 검증한 것은 우리 HTTP 목(`@mcpeak/mock`)까지다. 이 구분을 흐리지
않는다 — 소스를 읽은 것도, 우리 목에 붙여 본 것도, 남의 서버에 붙여 본 것과는 다른 근거이고,
이 표에서 "실측" 은 마지막 것에만 쓴다.
...
**원격(Streamable HTTP) 서버는 후보에서 아예 뺐다.** 이 선정 시점(§1.2 2행 · #137)에는 CLI 로
붙을 방법이 없어 stdio 로 띄울 수 있는 서버만 고를 수 있었다. 2026-08-23 에 `--url` 이 배선됐으므로
**다음 선정부터는 원격 서버를 후보에 넣을 수 있다.** 위 표본은 그 전에 고른 것이라 그대로 둔다.
...
### 1.7 `examples/zod-notes-server`: 자체 제작, CI 상시 (2026-09-08)
...
### 1.8 `mcp-server-git`: 실패 요약 절의 실환경 확인 (2026-09-10)

#446 의 완료 조건 C15 다. 툴 12개 · 축 59개를 값 교정 단계별로 세 번 돌려, 화면의 실패 요약
절이 실패 수를 그대로 따라가는지 봤다. 샌드박스는 저장소 밖 임시 디렉터리이고 초기화
스크립트를 두 번 돌려 HEAD 가 `3287e280…` 로 같은 것을 먼저 확인했다.

| 단계 | 판정 | 실패 요약 절 |
|---|---|---|
| 교정 전 | `47 passed, 12 failed  (59 total)` | 12행. 행마다 서버가 준 이유 |
| `repo_path` 35개 교정 후 | `57 passed, 2 failed  (59 total)` | 2행. **행마다 이유가 다르다** |
| `target` · `files` 7개 추가 교정 후 | `59 passed  (59 total)`, 종료 코드 0 | 나오지 않음 |

**중간 단계가 ADR-0092 의 R2 선택을 실측으로 뒷받침한다.** 두 행이
`Ref 'example' did not resolve to an object` 와 `Cmd('git') failed due to: exit code(128)` 로
갈렸는데, 진단 `message` 를 실었다면 둘 다 `정상 응답을 기대했지만 오류 응답을 받았습니다.` 로
같았을 것이다.

교정 전 12행의 글은 경로를 포함해 약 97자였고 상한 100자에 걸리지 않았다. `examples/weather-server`
대상 도그푸딩 실행(3 / 3 통과)에서는 절이 나오지 않고 거절 근거 미확인 고지가 요약 줄 바로
뒤에 그대로 붙는 것도 확인했다.
