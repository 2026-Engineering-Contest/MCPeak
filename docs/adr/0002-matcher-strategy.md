...
- 단점
  - **adapter 구현 방식 두 가지가 각각 우리 핵심 가치 하나씩을 잃는다.**
    ...
    - 케이스마다 Vitest test를 만들면 화면은 좋아지지만 실행 통제권이 Vitest로 넘어간다.
      한 파일 안의 test는 기본값에서 선언 순서대로 순차 실행되므로 케이스 순서 자체는
      기본 설정에서 지켜진다. 다만 그 보장이 우리 코드가 아니라 사용자 설정에 놓이며,
      `test.concurrent`·`describe.concurrent` 한 줄로 깨진다.
...
녹화·재생 기능이 있는 것도 그래서다.

Vitest의 기본 동작을 정확히 적어 둔다. **파일 단위 병렬은 기본으로 켜져 있고, 한 파일 안의
test는 선언 순서대로 순차 실행된다.** 즉 케이스 순서는 기본 설정에서 지켜지며, 깨지는 것은
사용자가 `test.concurrent`·`describe.concurrent`를 명시적으로 쓸 때다. 그럼에도 두 가지가
남는다. 첫째, **파일 병렬은 옵트인이 아니라 기본값이다.** suite를 파일로 나누는 순간 각자
자기 MCP 서버 프로세스를 동시에 띄우거나 하나를 공유하며 요청이 섞인다. 둘째, 케이스 순서
보장의 소재지가 우리 코드에서 사용자 설정으로 옮겨간다. 우리 문서가 보장하는 것을 사용자가
한 줄로 깨뜨릴 수 있다면 그것은 보장이 아니다.
...
- 확정 방법: **머지만으로 승인 처리하지 않는다.** main 브랜치 보호에 필수 리뷰가 걸려 있지
  않아(`required_pull_request_reviews`가 비어 있다) 사람 승인 없이도 머지된다. 파트 ① 밖
  오너 최소 1명의 명시적 승인(§5-2)을 확인한 뒤에 상태를 `승인`으로 바꾸고 승인일과 승인한
  오너를 적는다. 그때까지는 머지되어도 `제안`으로 둔다.
