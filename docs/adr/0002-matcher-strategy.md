...
- 승인: 승인 (2026-08-17, @endl24 · 파트 ① 오너) — 조건 완화 경위는 아래 「승인 상태」 참조
...
- 확정 방법: **저장소의 통용 절차를 따랐다** — 파트 오너가 자기 파트의 결정을 확정한다.
...
#### 원래 걸었던 조건과, 그것을 완화한 이유

이 ADR 은 처음에 더 엄격한 조건을 걸고 있었다.

> **머지만으로 승인 처리하지 않는다.** main 브랜치 보호에 필수 리뷰가 걸려 있지 않아
> (`required_pull_request_reviews` 가 비어 있다) 사람 승인 없이도 머지된다. **파트 ① 밖 오너
> 최소 1명의 명시적 승인(§5-2)** 을 확인한 뒤에 상태를 바꾼다.

**2026-08-17 에 이 조건을 완화했다. 조용히 바꾸지 않고 경위를 남긴다.**

**ⓐ 이 조건은 저장소에서 이 ADR 계열에만 걸려 있었다.** 같은 날 ADR 36건을 전수 확인한 결과,
`채택` 으로 올라간 16건 중 **파트 밖 오너의 명시적 승인을 받은 것은 0건**이다. 12건은 작성자가
같은 날 자기 이름으로 승인 처리했고(`0009`·`0021`~`0027`·`0031`~`0034`), 4건은 승인 줄 없이
상태만 올렸다(`0005`·`0007`·`0029`·`0030`, PR #116). **통용 절차는 파트 오너의 자기 확정이다.**

**ⓑ 조건이 구조적으로 충족 불가에 가까웠다.** 파트 ① 은 3명 공동 소유라(§2.1) 내부 승인으로는
조건을 못 채운다. 파트 밖 오너는 `@storyrago`·`@ddxng5` 둘뿐이고, 요청(이슈 #133, 2026-08-17)에
**코멘트 0건**으로 응답이 없었다. 코드 동결(2026-08-24)까지 7일이라, 이 조건을 유지하면 네 건이
동결 시점까지 `제안` 으로 남고 그 위에 이미 나간 구현만 남는다.

**ⓒ 그러나 이 조건이 지적한 문제는 사라지지 않았다.** `required_pull_request_reviews` 는 여전히
비어 있고, 2026-08-17 기준 PR #134·#139·#140·#141·#142 **다섯 건이 연속으로 사람 리뷰 0건으로
머지됐다.** 조건을 완화한 것은 문제가 해결돼서가 아니라 **혼자 지키는 규칙으로는 해결되지 않기
때문**이다. 브랜치 보호 설정과 §5-2 의 실효성은 팀 차원의 미결 과제로 남는다(이슈 #133 · #2).

**이 절을 지우지 않는다.** 무엇을 걸었고 왜 내렸는지가 사라지면 이 ADR 이 지적한 것도 함께
사라진다.
