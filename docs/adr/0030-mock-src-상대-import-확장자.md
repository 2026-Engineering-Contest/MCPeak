...
- 상태: 제안 — #110 이 정해지기 전까지 확정하지 않는다. 그 선택지 중 둘(픽스처가 `dist` 를 물게
  하기 · 파일을 다시 합치기)은 이 결정을 뒤집는다.
- 날짜: 2026-08-15
...
맞다. `tsconfig.base.json` 의 `paths` 는 `@ohmymcp/mock` 을 `packages/mock/src/index.ts`
(소스, 빌드 산출물이 아니다)로 매핑한다. 그래서 `@ohmymcp/mock` 을 TS 로 import 하는
...
"@ohmymcp/mock"` 하나만 추가하고 `tsc --noEmit` 을 돌리면:
...
지금은 `core`·`runner`·`generate`·`record`·`cli` 다섯 곳 모두 `@ohmymcp/mock` 을 workspace
