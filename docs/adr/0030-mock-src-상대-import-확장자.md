...
**후속 과제 (대체됨)** — 다른 패키지가 소스를 빌드 없이 raw node 로 돌리는 테스트를 도입하면
그때 이 관례를 저장소 전체로 통일할지 다시 검토한다. 지금은 이런 테스트를 가진
패키지가 `mock` 뿐이라 통일할 대상이 없다.

> 이 과제는 [ADR-0055](./0055-mock-src-가-저장소-공통-import-관례로-돌아온다.md) 로 넘어갔다.
> 통일할 대상은 `.ts` 확장자가 아니라 **리졸버 훅**이다 — `mock` 은 더 이상 다른 관례를 쓰지
> 않는다.

**후속 과제 (추가 · 해소됨)** — `@ohmymcp-hsu/mock` 을 TS 로 import 하는 첫 소비자 패키지가 생기면,
...
> **이 안내를 따르지 마라.** `#110` 은 어느 쪽도 택하지 않았다. ADR-0055 가 `mock` 의 소스에서
> `.ts` 확장자를 없앴으므로 **소비자는 아무 설정도 추가할 필요가 없다.** `tsconfig.base.json` 을
> 건드릴 일도 없어졌다.
