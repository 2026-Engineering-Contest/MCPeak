...
**A안: 스코프까지 개명한다.** 288 파일을 고치고 일곱 패키지를 `@mcpeak/*` 로 새로 발행한 뒤
옛 여섯을 정리한다. CLI 를 무스코프 `mcpeak` 으로 둘지는 위 「이름 확보 상황」 때문에
따로 갈린다 — 「결정」에서 스코프 안으로 넣었다.
...
- `bin` 이 `mcpeak` 이라 설치 후 치는 명령은 짧게 유지된다. 스코프는 설치 한 줄에만 나온다.
- CLI 를 처음으로 발행하게 되므로 `dashboard` 의 E404 결함이 함께 해소된다.

**받아들이는 비용**

- **288 파일** 치환. 일곱 패키지 전부를 건드린다. 아래 「소유권 규칙을 벗어나는 것」 참조.
...
**소유권 규칙을 벗어나는 것 — 알면서 벗어난다**

이 결정을 실행하는 PR 은 `CONTRIBUTING.md` §2.2 의 세 번째 항목에 **정면으로 어긋난다.**

> `cli` 와 `fixtures` 는 공용이지만, **한 PR에서 여러 오너의 영역을 동시에 건드리지 않는다.**

**§2.2 의 예외에 해당한다고 주장하지 않는다.** 그 예외는 "빌드가 **이미** 깨져 전원이 막힌
경우" 를 가리키고, 이 경우는 "쪼개면 빌드가 깨지는" 경우다. 시제가 다르다.

**쪼갤 수 없는 이유는 패키지 이름과 workspace 의존성이 원자적이기 때문이다.**
`packages/mock` 만 `@mcpeak/mock` 으로 바꾸면 `cli` 와 `dashboard` 의 `package.json` 이
여전히 `"@ohmymcp-hsu/mock": "workspace:*"` 를 가리켜 pnpm 해석이 실패하고, 그 시점부터
`build`·`typecheck`·`test` 가 전부 죽는다. 중간 커밋이 성립하지 않는다. `tsconfig.base.json`
의 경로 매핑, `vitest.config.ts` 의 alias, `.changeset/*.md` 의 대상 패키지명도 같은 이유로
함께 움직여야 한다.
...
PR #213 은 승인 0 건, `mergeStateStatus: BLOCKED` 상태에서 관리자 권한으로 머지했다.
코드 동결(2026-08-24)까지 남은 시간과 발행 순서(개명 → 버전 PR 재생성 → 일곱 패키지 발행 →
검증 → 옛 이름 삭제)를 고려한 릴리스 담당의 판단이다.

**이 문단을 지우지 않고 남기는 이유는, 지켜지지 않은 조건을 지켜진 것처럼 적으면 다음 사람이
그것을 선례로 인용하기 때문이다.** 위 §2.2 이탈은 "예외 조건에 준해 처리한다" 는 전제로
정당화됐는데, 세 조건 중 **오너 승인 한 가지가 실제로는 충족되지 않았다.**

- 머지 후 오너들이 이의를 제기하면 되돌린다. `release` scope 단일 커밋이라 revert 가 깨끗하다.
- **이 예외는 이 PR 한정이다.** 다음 개명·마이그레이션에서 "0050 때도 승인 없이 머지했다" 를
  근거로 쓸 수 없다.

**적용 범위: 개명 한정이다.** 기능 변경·리팩터링·버그 수정에는 적용하지 않는다. 이후 이 ADR 을
근거로 "개명 때 됐잖아" 를 인용할 수 없다.

기여 이력은 커밋 scope `release` 로 갈린다. CLAUDE.md 가 `release` 를 "소유 패키지가 없는
작업" 으로 정의하므로 `git log --grep "(mock)"` 같은 개인 기여 집계에서 빠진다. 소유권 규칙이
지키려는 것이 그것이며 이 방식으로 충족된다.
