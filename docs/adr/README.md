...
| [0018](./0018-junit-리포터-cli-노출-방식.md) | JUnit 리포터를 `--junit <path>` 로 노출한다 | cli | 제안 |
...
그리고 **그 0017 이 또 충돌했습니다.** JUnit 리포터 ADR 이 0017 을 잡은 채 리뷰 중이었는데, 그
사이 승인 지문 ADR 이 0017 로 재번호되어 먼저 머지됐습니다. 같은 선례를 따라 뒤에 온 JUnit
리포터 ADR 을 [0018](./0018-junit-리포터-cli-노출-방식.md) 로 옮겼습니다. 0014 → 0016 → 0017
→ 0018 로 세 번 연속입니다.

재번호는 그때그때의 땜질일 뿐 원인은 그대로입니다. 브랜치마다 독립적으로 다음 빈 번호를 집는
방식이라, 병렬로 도는 브랜치 수만큼 같은 번호가 잡힙니다. 예고한 충돌이 그대로 일어났다는 것이
이 방식으로는 못 막는다는 증거입니다. **번호 충돌을 해소한 PR 이 만든 번호가 다시 충돌했다는
점이 특히 분명한 신호입니다** — 해소 작업 자체가 새 충돌원이 되는 구조입니다. 번호 배정 규칙은
이 색인과 별개로 정해야 합니다.
