...
## 구현 중 드러난 것 (2026-09-21, 사후 기록)

이 계획서 자체가 놓친 결함 넷과 매처 함정 하나가 구현 도중 드러나 고쳐졌다. 원문은 지우지
않는다 — 계획서를 코드로 옮길 때 "적힌 대로 옮기면 되는가" 를 다시 검증하지 않고 넘어가면
같은 자리에서 같은 값을 밟는다. 아래는 뒷사람이 같은 함정을 다시 밟지 않게 하는 기록이다.

1. **`handleStartRelay`(`packages/dashboard/src/server/routes.ts`)가 후보 env 를 구해
   놓고 버렸다.** 주석은 "중계기에 물린다" 인데 코드는 존재 확인에만 쓰고 실제로 물리지
   않았다. 중계기 자식은 자기 `process.env` 에서 `--env` 이름을 찾으므로, 값이 안 가면
   relay 가 기동 줄을 내기 전에 죽고 대시보드는 15 초 뒤 「서버 명령이 stdio MCP 서버가
   맞는지 확인하세요」라고 답한다 — 원인을 잘못 가리키는 문장이다. `fix(dashboard):
   4 단계 중계기에 후보 env 값을 실제로 물린다`(46ddcea)에서 `relaySession.start()` 가
   `candidateEnv` 를 받아 `{ ...process.env, ...candidateEnv }` 를 자식에 넘기도록
   고쳤다(ADR-0105).
2. **`relay-lines.ts` 의 응답 파싱 분기에 `case` 처리가 빠져 있었다.** 타입
   (`RelayResponseEvent` 변형)에는 선언돼 있는데 파서 구현에는 없어서, 꼬리표가 응답
   줄에서 통째로 사라질 자리였다. `fix(dashboard): 중계 세션 누수·SSE 재개·기동 진단을
   4 건 고친다`(bfbf0e0)에서 요청·응답 양쪽 분기에 `case` 를 옮겨 실었다.
3. **`useEffect(() => () => void closeRelay(), [])` 가 첫 렌더의 클로저를 붙잡는다.**
   `relay` 가 상태(state)면 언마운트 정리 함수는 마운트 시점에 캡처된 값(항상 `null`)을
   본다 — **언마운트에서 아무것도 닫지 않는다.** 화면을 떠나도 중계기와 사용자 서버가
   계속 뜬 채로 남는다. `feat(dashboard): 3 단계 체크박스와 4 단계 실제 응답 화면을
   붙인다`(d41fd8f)에서 살아 있는 중계기를 `state` 대신 `ref`(`relayRef`)로 들도록
   맞춰 이 함정을 피했다.
4. **`openRelay` 에 가드가 없어 중계기가 두 번 열릴 수 있었다.** 덮이면 앞 중계기 id 를
   되찾을 길이 없어 아무도 닫지 못하고, 사용자 서버가 고아로 남는다. 도달 경로는 닫기
   실패를 삼킨 「이전」 뒤의 「다음」, 그리고 `POST /api/relay` 가 풀리기 전의 「이전」 —
   이 기능이 막으려던 「서버 두 벌」 상태를 정확히 네비게이션으로 재현하는 자리였다.
   `fix(dashboard): 중계기가 두 번 열리는 구멍을 막고 「이전」의 닫기 실패를
   말한다`(2fa7b84)에서 `relayRef.current !== null || relayOpeningRef.current !== null`
   가드를 추가하고, 「이전」에서 닫기가 실패하면 화면에 알리도록 했다.

**매처 함정 하나도 같은 계열이다**(§2 거짓 신호 표의 `toThrow` 부분 일치 항목 — 이 계획서
1799 행의 콜아웃 — 과 같은 자리에 한 줄 더한다). 필드 부재를 `toEqual` 로 단언하면
`{case: undefined}` 를 `{}` 와 같다고 본다. 그래서 구현이 `case` 를 항상 `undefined` 로
싣도록 퇴화해도(위 2번 결함이 정확히 이 모양이었다) 테스트는 계속 초록이다. `test(dashboard):
필드 부재 단언을 toStrictEqual 로 바꿔 회귀를 실제로 잡게 한다`(5562512)에서
`toStrictEqual` 로 바꿔 실제로 빨개지는 것을 확인한 뒤 다시 고쳤다.
