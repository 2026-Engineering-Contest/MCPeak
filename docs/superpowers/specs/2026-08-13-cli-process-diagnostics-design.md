...
4. 판정과 종료 코드를 바꾸지 않는다. 기존 테스트가 전부 그대로 통과한다.
...
세 번째 줄은 프로세스가 아직 살아 있는 경우다(실행 중 오류로 중단된 경로). 이 정보 자체가
"서버는 죽지 않았다" 는 단서이므로 생략하지 않는다.
...
`renderReport` 와 `test-command.ts` 가 쓰는 것과 **같은 규칙**을 쓴다. 즉 코드포인트가
