...
실패한 케이스만 한자리에 모으는 절이다(#446 · ADR-0092). 케이스가 수십 개인 서버에서 실패가
통과 줄 사이에 묻히는 것을 막는다. **요약 줄 바로 앞**이고, 그 뒤 빈 줄 하나가 요약 줄과의
간격이다.

`failed` 와 `timedOut` 이 **둘 다 0이면 한 줄도 내지 않는다.** 거절 고지·크기 고지와 같은
규칙이고, 그래야 전부 통과한 실행의 출력이 이 절이 생기기 전과 바이트 그대로다.
...
머리글은 `실패한 케이스` 고정 문안이다. 요약 행은 아래 형태다.

```
{두 칸}{mark} {caseId 패딩}{두 칸}→ {진단 글}
```

머지된 실제 출력이다. `packages/runner/tests/reporter.test.ts` 의
`describe("실패한 케이스 절")` 이 글자 그대로 고정한다.
...
중간 층위임을 보인다. 거절 고지·크기 고지의 첫 줄이 쓰는 값과 같다.

**절에 싣는 상태는 `failed` 와 `timedOut` 둘뿐이다.**

| status | 절에 넣는가 | 이유 |
|---|---|---|
| `passed` | 아니오 | 절의 존재 이유가 실패를 고르는 것이다 |
| `failed` | 예 | 본체다 |
| `timedOut` | 예 | 사용자가 손대야 하는 실패다. 기호가 `⧖` 라 목록 안에서 구분되고, 요약 줄이 `timed out` 을 따로 센다 |
| `cancelled` | 아니오 | 서버 판정이 아니라 사용자가 멈춘 사실이다. 중단 줄이 이미 그것을 말한다 |
| `notRun` | 아니오 | 실행되지 않았다. 옮길 진단이 없다 |

`mark` 는 §5.2 의 기호 표를 그대로 쓴다. 새 기호를 만들지 않는다.

**`caseId` 열은 절에 실린 케이스끼리만 맞춘다.** 보고서 전체 폭에 맞추면 긴 id 를 가진 통과
케이스가 절 안에 빈 공백을 만든다. 단언 타입 열을 케이스 안에서만 맞추는 §5.4 의 규칙과 같은
판단이다.
...
**진단 글은 100 코드 포인트에서 자른다.** 넘으면 앞에서 99 코드 포인트를 취하고 `…`(U+2026)을
붙여 정확히 100 코드 포인트로 만든다. 상한 상수는 `MAX_RECAP_TEXT_CHARS` 다.

- 절의 값은 한 케이스당 한 행이다. 200자짜리 위반 문장이 들어오면 80열 터미널에서 한 행이 세
  줄로 접히고, 실패 12건이면 절이 36줄이 되어 목적이 뒤집힌다.
- 100 은 실측에서 나온 가장 긴 실패 첫 줄(경로를 포함해 약 95자)이 잘리지 않는 값이다. 진단 값
  자체는 `MAX_VALUE_STRING_CHARS`(200)로 이미 한 번 잘려 오므로 여기 상한은 그 절반이다.
- **자르기가 정보를 잃지 않는다.** 원본은 같은 화면 위 케이스 블록에 온전히 있다. 이 행은
  가리키는 자리이지 판정 근거가 아니다.

**순서는 `report.cases` 순서다.** 다시 정렬하지 않는다. 위 목록에서 본 순서와 같아야 눈이
대응시킬 수 있고, 정렬은 결정론성에 새 변수를 들인다. 이스케이프가 먼저, 자르기가 그다음,
색상 삽입이 마지막이다(§6).
...
| 실패 요약 절의 `✗` `⧖` | 케이스 줄과 같은 SGR (빨강, 노랑) |
| 실패 요약 절의 머리글 · `caseId` · `→` · 진단 글 | 없음 |
| 그 외 | 없음 |

각 구간은 `\u001b[0m` 으로 닫는다. 진단 문장 본문과 위반 줄에는 색을 넣지 않는다. 색이 많으면
읽기 어렵고, 색 없는 기본 출력과 구조가 달라져 테스트가 두 배가 된다. 실패 요약 절이 새 SGR
코드를 만들지 않는 것은 `dashboard` 의 `ansiToHtml` 이 이미 아는 코드만 받게 하려는 것이기도
하다.
