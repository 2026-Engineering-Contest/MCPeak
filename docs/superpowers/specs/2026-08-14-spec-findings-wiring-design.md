...
**케이스 사이 순서는 `finalReport.cases` 의 순서다.** 두 검사 결과를 이어 붙인 순서가 아니다.
검사 결과를 먼저 합치고 `caseId` 로 묶으면, 앞 케이스에 단언 실질성 finding 만 있고 뒤 케이스에
입력 계약 finding 이 있을 때 뒤 케이스가 먼저 나온다. 그래서 실패한 케이스의 버킷을 보고서
순서로 먼저 만들고 거기에 finding 을 채운다. 블록 안 순서는 `runner` 가 준 순서 그대로다.
소비자가 재정렬하지 않는다.
