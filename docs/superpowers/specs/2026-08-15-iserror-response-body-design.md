...
| `form === "text"` | 개행으로 나눠 줄마다 항목 하나로 싣는다. 빈 줄은 버린다 |
...
- `MAX_VALUE_STRING_CHARS` 로 자른다. `diagnostics.ts` 에 이미 있는 자르기 헬퍼를 쓴다. 새로
  만들지 않는다. **자르기는 줄로 나누기 전 본문 전체에 건다.** 줄마다 따로 자르면 긴 응답에서
  총량이 안 잡힌다.
- 줄 안의 글자는 손대지 않는다. 서버가 본문 안에서 `→` 를 글머리로 쓰면 `→ → ...` 로 나온다.
  그것은 서버가 보낸 글자를 우리가 지우지 않는다는 뜻이라 맞다.
...
바뀐 뒤. `examples/weather-server` 에 `{"city":"example"}` 를 보낸 실제 출력이다.
...
`→` 가 두 번 나오는 것은 이 예제 서버가 자기 오류 문장 안에서 `→` 를 글머리로 쓰기 때문이다.
앞의 것이 리포터가 붙인 것이고 뒤의 것은 서버가 보낸 글자다.
