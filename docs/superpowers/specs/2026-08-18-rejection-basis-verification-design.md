...
3. `mcpeak test` 요약에 확인 못 한 건수가 나온다. 0건이면 그 줄이 안 나온다.
...
- **응답 본문은 상한·확인·기본 거부로 다룬다.** 200자 상한(`clampObservedText`), 승낙 전
  본문 한 줄씩 표시, 기본값 `N` 인 확인 질문 셋이다. stderr 의 `--no-stderr` 는 기본으로 싣고
  빼는 옵트아웃이지만, 여기는 승낙 전까지 안 나가는 기본 거부다.
