...
3. 케이스마다 `claude` 를 한 대씩 띄우되 **동시에 도는 수는 6 대로 막는다**(ADR-0106,
   `RELAY_AI_CONCURRENCY`). 각자 자기 케이스 꼬리표가 붙은 URL 을 받는다 —
   `http://127.0.0.1:<포트>/mcp?case=<케이스id>`.
   > **정정(2026-09-22).** 원래 이 줄은 「케이스 수만큼 동시에」였다. 82 케이스 스위트에서
   > `claude` 82 대가 한꺼번에 떠 12 코어 머신 부하가 73 까지 올라갔다. 상한과, 닫을 때
   > 대기열·진행 중인 AI 를 함께 중단하는 계약은 ADR-0106 과
   > `2026-09-22-dashboard-relay-concurrency-limit-design.md` 가 대체한다.
