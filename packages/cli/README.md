...
현재 명세에서 계산한 지문과 저장된 `approval.fingerprint`가 불일치하면 전부 통과했는지 실패가
있는지에 맞춰 상황을 구분해 알립니다. 지문 두 개만으로는 구체적인 변경 내용을 복원할 수 없으므로
버전 관리에서 명세를 비교하도록 안내하고, 의도한 변경이면 같은 설정의 `mcpeak generate`에
`--force`를 붙여 다시 생성·승인하도록 안내합니다.
