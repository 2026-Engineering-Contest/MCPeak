...
const TEST_OPTIONS = `옵션:
  ...
  --determinism         스위트를 2회 실행해 결과가 같은지 확인합니다. 툴을 2회
                        ...
                        비차단 진단입니다 — 차이를 찾아도 종료 코드는 시험 판정
                        그대로이며 이 진단만으로 1 이 되지 않습니다. CI 를 막으려면
                        --json 의 determinism 키를 보세요.
                        차이 지점의 양쪽 값을 화면에 싣습니다. \`token\`·\`apiKey\` 같은
                        이름의 자리는 가려지지만, 서버가 결과를 JSON 문자열로 만들어
                        text 블록 하나에 실으면 그 안의 비밀값은 이름이 없어
                        가려지지 않습니다. 실행마다 비밀값을 새로 발급하는 서버라면
