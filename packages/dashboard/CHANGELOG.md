...
- 2cb8a34: Generate 실행 화면에서 사용자 AI 요청과 provider 결과를 터미널 출력의 발생 순서에 맞춰
  `사용자 질문`·`AI 응답` 라벨로 구분합니다. 별도 대화 카드에 응답을 중복 표시하지 않으며,
  응답을 기다리는 동안에는 같은 터미널 흐름에서 `AI가 답변 중입니다...` 상태를 보여 줍니다.
- ecf3f3d: 대시보드 시각 토큰을 `theme.css` 한 곳으로 모읍니다. 테마가 갈리는 값은 `light-dark()` 로 한 번만
...
- d77a728: `.mcp.json` 의 `env` 를 자식 서버에 넘깁니다. 이름은 `--env <NAME>` 으로 argv 에 실리고 값은 대시보드 서버 프로세스 안에서만 해석되므로, 브라우저에도 실행 명령에도 비밀값이 남지 않습니다.
...
- 3ae18bf: Generate의 AI 모델 입력을 도구별 선택 목록으로 바꿉니다. Claude에서는 Sonnet·Haiku·Opus를,
...
- 8f330d5: 녹화본을 재생하는 Replay 탭을 추가합니다. 프로젝트 아래에서 찾은 External 세션을 녹화 시각
...
- b05cb58: 대시보드의 generate 검토 하위 입력에 `검토 메뉴로 돌아가기` 버튼을 추가합니다. 모델, AI 요청,
  ...
  수 있습니다.
- 9c37e9e: 대시보드 화면의 위계를 다시 잡습니다(#459).
...
- 79b990c: 카드 안에서 목록이나 여러 컨트롤을 묶는 머리글(접속 · 서버 · 이 서버의 스위트 · External 세션 ·
...
- b3f4101: Generate 검토 메뉴와 AI 입력 영역을 강조하고, 전송 승인 후 다음 검토 메뉴가 도착할 때까지 스위트 생성 상태를 표시합니다.
- f2f5d02: dashboard: 테마 버튼을 라이트·다크 2값 토글로 바꿉니다. `시스템`은 선택지에서 빠지고 "아직
...
- 3bfb032: 대시보드 버튼과 카드를 공용 컴포넌트로 모읍니다. 흩어져 있던 버튼 19곳(프라이머리 10 · 보조 9)이
...
- cc8a038: 홈을 서버부터 고르는 3단계 마법사로 바꿉니다. 1단계에서 서버(접속 방식 포함)를 고르고,
...
- 8adc58a: Runs 목록과 실행 상세 머리에 그 run 이 무엇을 무엇으로 돌렸는지(스위트 경로와 서버 명령 또는
  HTTP URL)를 함께 보여줍니다. argv 에서 읽으므로 모르는 값은 지어내지 않고 비웁니다.
  사이드바 「Home」 은 「Test」 로 바뀝니다.

### Patch Changes

- 6520adc: dashboard의 사용하지 않는 mock 런타임 의존을 제거하고, 선언된 내부 의존과 실제 import 및 단방향 패키지 경계를 테스트로 고정합니다.
- 2d2c7c3: 대시보드가 사용자가 답한 값을 터미널 출력에 되비추고, generate 폼 4단계에 `--diagnose-rejections` 토글을 더한다.
...
- f63bebe: 입력 필드 프리미티브(`Field`·`INPUT_CLASS`·`Toggle`)를 `components/` 로 옮기고, 새어 나갔던
  복제 세 벌을 되돌립니다. repair 폼의 입력칸이 비활성일 때 흐려지지 않던 문제가 함께 고쳐지고,
  모든 입력칸이 버튼과 같은 포커스 링을 갖습니다.
- b788f6c: dashboard: 실행 화면의 터미널 출력 메타가 이벤트 수 대신 실제 줄 수를 보여 줍니다. `mcpeak test`
  는 출력을 모았다가 끝에 한 번 쓰므로 stdout 이벤트가 1개인데, 그 수를 세어 "줄" 이라고 부르는
  바람에 화면에 16줄이 보여도 `1줄` 이라고 표시됐습니다. 개행을 기준으로 세며 빈 줄도 포함합니다.
- fe28066: Generate 후속 AI 질문의 답변 대기 상태를 스위트 생성 상태와 구분하고, AI 전송 확인 화면의 색상 대비를 개선합니다.

  Repair 실행 시 provider별 지원 모델을 선택하도록 제한하고 provider를 변경하면 기존 모델 선택을 초기화합니다.

- 3306e13: 세 곳에 똑같이 복제돼 있던 세그먼트 컨트롤을 `SegmentedControl` 한 벌로 합치고, 아직 브라우저
  기본 아웃라인에 기대던 버튼·링크에 나머지와 같은 포커스 링을 줍니다. 키보드로 옮겨 다닐 때
  지금 어디에 있는지가 요소마다 다르게 보이던 것이 없어집니다.
- 742c8b0: 모든 질문 유형에 동일한 연보라색 배경과 입력 요소 스타일을 적용해 질문 패널의 디자인을 일관되게 표시합니다.
- b254935: 실행 화면의 터미널 출력이 새 줄을 따라갑니다. 본문은 높이가 묶인 자체 스크롤 영역이라
  따라가지 않으면 답변할 때마다 새 출력이 화면 밖 아래에 쌓였고, AI 에게 묻거나 검토 메뉴를
  고를 때마다 사용자가 다시 내려야 했습니다. 지난 출력을 확인하려고 위로 올려 둔 경우에는
  끌어내리지 않습니다.
- 2f1af69: 녹화 실패 사유 문구를 실제 원인에 맞춘다.
- 1c4c673: 실행 화면이 페이지를 스크롤하지 않습니다. 화면 높이를 뷰포트에 묶고 터미널 출력이 남는
  공간을 채우게 해, 로그가 아무리 길어져도 질문 패널이 늘 화면 안에 있습니다. 질문이 끝나면
  로그가 그 자리를 되찾습니다. 내용이 긴 다른 화면들은 스크롤 주체가 문서에서 본문 영역으로
  바뀔 뿐 보이는 동작은 같습니다.
- Updated dependencies [eaa917e]
- Updated dependencies [a1b3af5]
- Updated dependencies [e2fd278]
- Updated dependencies [7f51354]
- Updated dependencies [72fc934]
- Updated dependencies [052f0c7]
- Updated dependencies [b05cb58]
- Updated dependencies [2b9751e]
- Updated dependencies [625b9d6]
- Updated dependencies [adfb2b4]
- Updated dependencies [b3c8997]
