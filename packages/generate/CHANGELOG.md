...
- 2b9751e: 진단 요청에 도구 출력 계약과 stderr 범위를 싣는다
...
- 81c2828: 중첩 객체와 배열 원소의 입력 계약 위반 케이스를 만든다
- c39c6d5: 선택 필드 아래 중첩 경로의 위반 케이스도 만든다
- da8dcba: `tools/list`의 `outputSchema`를 생성 명세에 보존하고 정상 케이스의
  ...
  알립니다.
- a4d62e5: 범위 위반을 상한과 하한으로 나눠 만들고 상한 경계 정상 케이스를 더한다
...
- 45f8b05: 정상 입력의 enum·boolean·선택 필드 분기를 케이스로 만든다
- c6b86a0: 픽스처로 값을 지정하고 값의 출처를 기록한다

### Patch Changes

- 269d56e: format 표의 문서용 예약 값으로는 선택 필드 '있음' 케이스를 만들지 않는다
