...
- **생성 시 의존:** `@mcpeak/core`, `@mcpeak/runner`
- **생성 결과 실행 시 의존:** `@mcpeak/runner`
...
```ts
import { createBaselineSuite } from "@mcpeak/generate";
...
```ts
import { generateTests } from "@mcpeak/generate";
...
`additionalProperties`, `propertyNames`, 로컬 `$ref`(`$defs`·`definitions`), `anyOf`, `oneOf`
입니다. `allOf`·`not`·`if`·`patternProperties`·배열 형태 `type`·튜플 `items`·원격 `$ref` 는 아직
...
```ts
import { runSuite } from "@mcpeak/runner";
