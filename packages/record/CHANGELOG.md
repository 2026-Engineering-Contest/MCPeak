...
- 38a3c04: `npx` 같은 Node 런처를 거쳐 띄운 서버의 외부 호출이 하나도 녹화되지 않던 문제를 고칩니다.
...
- 008e6b8: 범위 밖 호출 관측이 중간에 낀 Node 런처(`npx` 등) 자신의 트래픽을 세던 문제를 고칩니다.
...
- 6cc18f3: 녹화 세션에 저장되는 실행 출처(`origin.args`)에서 비밀 모양의 인자를 가립니다. 지금까지는
