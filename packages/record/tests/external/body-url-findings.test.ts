...
const record = (
  ...
  matchKey: string,
  bodyUrls: { echoed: string[]; other: string[] },
): void => {
...
describe("Record Engine — 세션 전체에서 어떻게 세는가", () => {
  ...
  it("서로 다른 지문의 개수를 센다 — 반복 호출로 같은 URL 이 여러 번 나와도 1건이다", () => {
    ...
    expect(engine.finish("completed").bodyUrls).toEqual({ echoed: 0, other: 2 });
  });
  ...
  it("지문을 안 보낸 interaction 은 셀 것이 없는 것으로 다룬다", () => {
    ...
    expect(engine.finish("completed").bodyUrls).toEqual({ echoed: 0, other: 0 });
  });
  ...
  it("한 지문은 정확히 한 갈래에만 센다 — 두 갈래에 중복으로 세지 않는다", () => {
    ...
    expect(engine.finish("completed").bodyUrls).toEqual({ echoed: 1, other: 0 });
  });
  ...
  it("같은 지문이 두 갈래로 나뉘어 와도 순서와 무관하게 echoed 가 이긴다", () => {
    ...
    const expected = { echoed: 1, other: 0 };
